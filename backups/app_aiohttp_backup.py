...
from aiohttp import web, ClientSession, TCPConnector, ClientTimeout, ClientConnectorError, ClientOSError
    ...
    async def initialize(self):
        ...
        if self.connector is None:
            ...
            if not config.BACKEND_SSL_VERIFY:
                # 当BACKEND_SSL_VERIFY=False时，总是禁用SSL验证
                # 这适用于：1) HTTPS后端连接 2) 后端服务器自身发起的HTTPS请求
                ssl_context = ssl.create_default_context()
                ssl_context.check_hostname = False
                ssl_context.verify_mode = ssl.CERT_NONE
                base_logger.info("已禁用SSL证书验证（适用于自签名证书或测试环境）")
            
            self.connector = TCPConnector(
                limit=config.HTTP_CONNECTOR_LIMIT,
                limit_per_host=config.HTTP_CONNECTOR_LIMIT_PER_HOST,
                ttl_dns_cache=config.HTTP_DNS_CACHE_TTL,
                use_dns_cache=True,
                keepalive_timeout=config.HTTP_KEEPALIVE_TIMEOUT,
                enable_cleanup_closed=True,
                force_close=False,
                family=0,  # 允许IPv4和IPv6
                ssl=ssl_context,  # 应用SSL上下文
            )
        ...
        if self.session is None:
            ...
            self.session = ClientSession(
                ...
                timeout=timeout,
                headers={'User-Agent': 'OptimizedProxyServer/2.0'},
                read_bufsize=config.BUFFER_SIZE,
                max_field_size=8192,
                max_line_size=8192
            )
