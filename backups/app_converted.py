...
class HTTPClientManager:
    ...
    async def initialize(self):
        ...
        if self.client is None:
            limits = httpx.Limits(
                ...
                max_keepalive_connections=config.HTTP_CONNECTOR_LIMIT_PER_HOST
            )
            ...
            verify = not (not config.BACKEND_SSL_VERIFY)
            ...
            base_logger.info(f"HTTP客户端管理器初始化完成 [PID: {os.getpid()}]")
    ...
    async def close(self):
        ...
        if not self._closed and self.client:
            ...
            self._closed = True
            base_logger.info(f"HTTP客户端管理器已关闭 [PID: {os.getpid()}]")
...
# === 流式代理函数（性能优化核心）===
# === 增强的流式代理函数（核心修复）===
async def stream_proxy_response(remote_url: str, headers: Dict[str, str], request, logger, file_type: str = "default", uid: str = None, session_id: str = None) -> StreamingResponse:
    """流式代理响应，增强错误处理和稳定性"""
...
async def proxy_handler(request):
    ...
    if not skip_validation:
        ...
        if not is_allowed:
            ...
            return Response(content="Access Denied 100020 path", status=403)
        ...
        if is_m3u8:
            if not effective_uid:
                ...
                return Response(content="No valid UID for .m3u8 request", status=403)
            
            if not uid or not expires or not token:
                logger.warning(f".m3u8 请求缺少 HMAC 参数: path={path}, key_path={key_path}")
                return Response(content=".m3u8 request missing required parameters (uid, expires, token)", status=400)
            
            hmac_valid = validate_token(uid, path, expires, token, logger)
            if not hmac_valid:
                logger.warning(f".m3u8 请求令牌无效或已过期: path={path}, key_path={key_path}")
                return Response(content=".m3u8 request token invalid or expired hmac", status=403)
            ...
            if not await check_m3u8_access_count(effective_uid, full_url, client_ip, user_agent, logger):
                ...
                return Response(content=f"Access Denied 100308 with uid:{effective_uid}", status=403)
    ...
    try:
        ...
        if ErrorHandler.is_client_disconnect_error(e):
            ...
            return Response(content=f"Proxy failed: {err_msg}", status=502)
...
async def add_ip_whitelist(request):
    ...
    try:
        ...
        if not api_key or api_key != "Bearer F2UkWEJZRBxC7":
            ...
            return JSONResponse(content={"error": "Invalid or missing API key"}, status=403)
        ...
        if not uid or not path or not target_client_ip or not user_agent:
            ...
            return JSONResponse(content={"error": "uid, path, clientIp, and userAgent are required"}, status=400)

        # Extract key path
        key_path = extract_match_key(path)
        if not key_path:
            logger.warning(f"Invalid key path extracted from {path}")
            return JSONResponse(content={"error": "Invalid path format"}, status=400)
        ...
        return JSONResponse(content={"error": "Invalid JSON data"}, status=400)
    except Exception as e:
        logger.error(f"add_ip_whitelist error: {str(e)}")
        return JSONResponse(content={"error": f"Failed to add IP to whitelist: {str(e)}"}, status=500)
...
async def probe_backend_file(request):
    ...
    try:
        ...
        if not path or ".." in path:
            return JSONResponse(content={"status": "error", "reason": "invalid_path"}, status=400)
...
async def monitor_dashboard(request):
    ...
    try:
        ...
        if os.path.exists(monitor_file):
            ...
            return Response(content="监控面板文件未找到", status=404)
    except Exception as e:
        base_logger.error(f"加载监控面板失败: {str(e)}")
        return Response(content=f"加载监控面板失败: {str(e)}", status=500)
...
async def serve_static(request):
    ...
    try:
        ...
        if not os.path.abspath(full_path).startswith(os.path.abspath(static_dir)):
            return Response(content="禁止访问", status=403)
        
        if not os.path.exists(full_path) or not os.path.isfile(full_path):
            return Response(content="文件未找到", status=404)
        ...
        return web.Response(body=content, media_type=content_type)
        
    except Exception as e:
        base_logger.error(f"服务静态文件失败: {str(e)}")
        return Response(content=f"服务文件失败: {str(e)}", status=500)


# === 创建应用函数 (gunicorn 入口点) ===
def create_app():
    """创建 aiohttp 应用实例 - gunicorn 入口点"""
    app = web.Application()
    ...
    # 添加路由并配置CORS - 注意顺序，通用路由应该最后添加
    cors.add(app.router.add_route("GET", "/health", health_check))
    cors.add(app.router.add_route("GET", "/stats", performance_stats))
    cors.add(app.router.add_route("POST", "/api/whitelist", add_ip_whitelist))
    cors.add(app.router.add_route("GET", "/debug/browser", browser_detection_debug))
    cors.add(app.router.add_route("GET", "/debug/cidr", cidr_debug))
    cors.add(app.router.add_route("GET", "/debug/ip-whitelist", ip_whitelist_debug))
    cors.add(app.router.add_route("GET", "/traffic", traffic_stats))
    cors.add(app.router.add_route("GET", "/debug/session", session_debug))
    cors.add(app.router.add_route("GET", "/probe/backend", probe_backend_file))
    # 监控面板路由
    cors.add(app.router.add_route("GET", "/monitor", monitor_dashboard))
    cors.add(app.router.add_route("GET", "/static/{path:.*}", serve_static))
    # 通用代理路由应该最后添加，因为它会匹配所有路径
    cors.add(app.router.add_route("GET", "/{path:.*}", proxy_handler))

    # 添加启动和关闭钩子
    app.on_startup.append(init_app_on_startup)
    app.on_shutdown.append(cleanup_app_on_shutdown)
...
if __name__ == '__main__':
    try:
        base_logger.info("🚀 优化版HMAC反向代理服务器启动中 (直接运行模式)...")
        web.run_app(app, host='0.0.0.0', port=7888, access_log=base_logger)
    except KeyboardInterrupt:
        base_logger.info("服务器被用户中断")
    except Exception as e:
        base_logger.error(f"服务器启动失败: {str(e)}")
    finally:
        base_logger.info("服务器已关闭")
